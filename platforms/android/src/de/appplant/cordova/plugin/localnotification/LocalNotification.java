...
public class LocalNotification extends CordovaPlugin {
    ...
    public static void add (Options options, boolean doFireEvent) {
        ...
        int tempDuration = options.getDuration();
        ...
        //if duration has been set, cancel the notificaiton after duration
        if(tempDuration != 0){
        	Intent cancelIntent = new Intent(context, ReceiverCancel.class)
        		.setAction("" + options.getId())
        		.putExtra(ReceiverCancel.OPTIONS, options.getJSONObject().toString());
        	
        	PendingIntent cancelPI = PendingIntent.getBroadcast(context, 0, cancelIntent, PendingIntent.FLAG_CANCEL_CURRENT);
        	am.set(AlarmManager.RTC_WAKEUP, triggerTime + tempDuration, cancelPI);
        }
